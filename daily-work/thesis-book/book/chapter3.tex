...
steps in image processing that must be performed in order to successfuly fit the shape of 
...
specifical class of objects that is defined in geometrical
...
to the contour points, in order to adapt the generical shape silhouette to 
the matching object. 
...
states that this approach allows to find succesfully a connected 1-pixel-thin skeleton,
...
seemed appropriate in theory, the costful reconnecting operations and the unnacurate
...
parrallel thinning algorithm is presented, that consist in two subiterations per main 
...
to avoid the requirement of using a parrallel computer without losing the time perfomance 
improvement, the distance map was used to discard unnecesary pixel checking (those
...
image that was previously identified in the binary image obtention process 
(see \ref{sec:metthres}) is succesfully \emph{skeletonized}.
...
of paths can be distinguished, in correspondance to the objects they represent,
...
the obtained subimages separately.\\

Another process of segmentation that is perfomed is the identification of single
...
removing subskeletons that have a size (number of pixels) lower than a small 
threshold. This allows to remove some slightly noisy regions and incorrect endpoints.
Once the skeleton is succesfully expanded, the endpoints of the skeleton are considered
worm endpoints. 
...
as one object in the binary image obtention process, so then in the skeletonization
...
Below in algorithm \ref{groupsegment} a pseudocode for this group segmentation 
...
connects them is still uknown. The optimization algorithm, covered in \ref{sec:metfit},
...
endpoints bounding), a path guessing algorithm was designed that performs a heuristical
...
the distance map value of the pixel. This can be expresed as below:
...
be hiden because of the overlapping, the number of pixels that have been added
...
of endpoints. Below, algorithm \ref{guess} presents a pseudocode for the path
guessing approach.
